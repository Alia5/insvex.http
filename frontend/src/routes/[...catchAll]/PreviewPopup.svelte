...
<script lang="ts">
  ...
  let {
    file = $bindable(),
    previewableItems = [],
  }: { file?: string; previewableItems?: string[] } = $props();
  ...
  const fileUrl = $derived(
    `${page.url.pathname}${page.url.pathname === "/" ? "" : "/"}${file}`,
  );
  ...
  onMount(() => {
    window.onresize = () => {
      isPortrait =
        window?.screen?.height > window?.screen?.width ? true : false;
    };
  });
  ...
  const calculateSizePrcnt = () => {
    ...
    if (fullSize[0] && fullSize[1]) {
      const sizePrcnt = [
        elementSize[0] / fullSize[0],
        elementSize[1] / fullSize[1],
      ];
  ...
  const previewElementObserver = browser
    ? new ResizeObserver(calculateSizePrcnt)
    : undefined;
...
<div class="popup-container {isFullscreen ? 'fullscreen' : ''}">
  {#if file && file.length > 0}
    ...
    ></button>
    <div
      class="container {isFullscreen ? 'fullscreen' : ''} {isPortrait
        ? 'portrait'
        : ''}"
