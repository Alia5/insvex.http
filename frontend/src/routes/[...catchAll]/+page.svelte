<script lang="ts">
  ...
  import PreviewPopup, {
    supportsMimeType as previewSupportsType,
  } from "./PreviewPopup.svelte";
  ...
  const thumbPrefixPath = $derived(
    `${thumbHost}${data.currentPath === "/" ? "" : data.currentPath}`,
  );

  let files: DirList = $state([]);
  let path: string | undefined = $state(undefined);
  let page = $state(1);
  const totalPages = $derived(data.dirList.totalPages);
  const unauthorized = $derived(data.unauthorized || false);
  let loading = $state(false);

  $effect(() => {
    const incomingPath = data.currentPath;
    const incomingPage = data.dirList.page;
    const incomingFiles = data.dirList.files || [];
    ...
    if (incomingPage < page || incomingPage <= 1) {
      page = incomingPage;
      files = [...incomingFiles];
      return;
    }
    ...
    if (incomingPage > page) {
      ...
      files = [
        ...files,
        ...incomingFiles.filter((file) => !existingPaths.has(file.path)),
      ];
      page = incomingPage;
    }
  });
...
{:else}
  <section onscroll={handleInfScroll}>
    ...
    <div class="load-more-container" no-js-hidden>
      {#if page < totalPages}
        <!-- <a href="?page={page + 1}">Load more</a> -->
        {#if !loadingMore}
          <button
            onclick={() => {
              loadingMore = true;
